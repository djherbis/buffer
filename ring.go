...
func (buf *Ring) Write(p []byte) (n int, err error) {
	...
	buf.L += int64(n)
	if buf.L > buf.Buffer.Cap() {
		buf.L = buf.Buffer.Cap()
	}
	return n, err
}
...
func (buf *Ring) Reset() {
	buf.Buffer.Reset()
	buf.L = 0
	buf.WrapReader = NewWrapReader(buf.Buffer, 0, buf.Buffer.Cap())
	buf.WrapWriter = NewWrapWriter(buf.Buffer, 0, buf.Buffer.Cap())
}
